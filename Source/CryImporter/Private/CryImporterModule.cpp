...
namespace CryImporter
{
    ...
    // GRP global vertical correction for this project setup.
    static constexpr float GRPGlobalZOffset = -12848.0f;
    static const FVector2D GRPGlobalXYOffset(-1386.0f, -566.0f);
    ...
    static FVector BuildFinalGRPPlacementLocation(const FVector& CryPos)
    {
        const FVector LocalPosition = ConvertCryPositionToUE(CryPos);
        return ApplyGlobalGroupYawToPosition(LocalPosition) + FVector(0.0f, 0.0f, GRPGlobalZOffset);
    }

    static FRotator BuildFinalGRPPlacementRotation(const FVector& CryAnglesDeg)
    {
...
bool FCryImporterModule::SnapSelectedActorsToLandscape()
{
    ...
    auto FindLandscapeHit = [](const TArray<FHitResult>& Hits, FHitResult& OutLandscapeHit) -> bool
    {
        for (const FHitResult& CandidateHit : Hits)
        {
            if (CandidateHit.GetActor() && CandidateHit.GetActor()->IsA<ALandscapeProxy>())
            {
                OutLandscapeHit = CandidateHit;
                return true;
            }
        }
        return false;
    ...
    for (AActor* Actor : SelectedActors)
    {
        ...
        for (AActor* IgnoredActor : SelectedActors)
        {
            if (IgnoredActor)
            {
        ...
        {
            bHitLandscape = FindLandscapeHit(Hits, LandscapeHit);
        }
        ...
        if (!bHitLandscape || !LandscapeHit.bBlockingHit)
        {
            continue;
        }

        Actor->Modify();
        Actor->SetActorLocation(FVector(Loc.X, Loc.Y, LandscapeHit.Location.Z));
        ++SnappedCount;
    }
...
void FCryImporterModule::ImportGRP(const FString& FilePath)
{
    ...
    ParseNode = [&](FXmlNode* Node)
    {
        ...
        if (Node->GetTag() == TEXT("Object"))
        {
            ...
            const FString ObjectId = Node->GetAttribute(TEXT("Id"));
            ...
            if (!PrefabPath.IsEmpty())
            {
                ...
                if (SelectedMesh)
                {
                    ...
                    const FVector LocalPosition = bHasExplicitPos
                        ? CryImporter::ConvertCryPositionToUE(CryPos)
                        : FVector::ZeroVector;

                    FVector CryAngles = FVector::ZeroVector;
                    const bool bHasAngles = CryImporter::ParseVector3Csv(AnglesString, CryAngles);
                    FRotator LocalRotation = FRotator::ZeroRotator;
                    if (bHasAngles)
                    {
                        LocalRotation = CryImporter::ConvertCryAnglesToUERotator(CryAngles);
                        if (CryImporter::bUseLegacyRotationFallback)
                        {
                            LocalRotation = CryImporter::ConvertCryAnglesLegacy(CryAngles);
                        }
                    }

                    FVector LocalScale3D(1.0f, 1.0f, 1.0f);
                    FVector CryScale = FVector::ZeroVector;
                    if (CryImporter::ParseVector3Csv(ScaleString, CryScale))
                    {
                        LocalScale3D = CryImporter::ConvertCryScaleToUE(CryScale);
                    }

                    const FTransform* ParentWorldTransform = (!ParentId.IsEmpty())
                        ? WorldTransformByObjectId.Find(ParentId)
                        : nullptr;

                    FVector Location = FVector::ZeroVector;
                    FQuat WorldRotationQuat = FQuat::Identity;
                    ...
                    if (ParentWorldTransform)
                    {
                        if (bHasExplicitPos)
                        {
                            Location = ParentWorldTransform->TransformPosition(LocalPosition);
                        }
                        else
                        {
                            Location = ParentWorldTransform->GetLocation();
                        }
                        WorldRotationQuat = ParentWorldTransform->GetRotation() * LocalRotation.Quaternion();
                        Scale3D = ParentWorldTransform->GetScale3D() * LocalScale3D;
                    }
                    else if (bHasExplicitPos)
                    {
                        Location = GlobalYawQuat.RotateVector(LocalPosition)
                            + FVector(CryImporter::GRPGlobalXYOffset.X, CryImporter::GRPGlobalXYOffset.Y, 0.0f)
                            + LandscapeCenterOffset
                            + FVector(0.0f, 0.0f, CryImporter::GRPGlobalZOffset);
                        WorldRotationQuat = GlobalYawQuat * LocalRotation.Quaternion();
                        Scale3D = LocalScale3D;
                    }
                    else if (!LayerName.IsEmpty())
                    {
                        if (const FVector* LayerFallbackLocation = LastKnownLocationByLayer.Find(LayerName))
                        {
                            ...
                            Scale3D = LocalScale3D;
                            UE_LOG(LogTemp, Warning,
                                TEXT("CryImporter: Object '%s' has no Pos. Using last known location from Layer '%s'."),
                                *FPaths::GetBaseFilename(PrefabPath),
                                *LayerName);
                        }
                        else
                        {
                            UE_LOG(LogTemp, Warning,
                                TEXT("CryImporter: Object '%s' has no Pos and no layer fallback for '%s'. Skipping object."),
                                *FPaths::GetBaseFilename(PrefabPath),
                                *LayerName);
                            return;
                        }
                    ...
                    {
                        UE_LOG(LogTemp, Warning,
                            TEXT("CryImporter: Object '%s' has no Pos and no Parent/Layer fallback. Skipping object."),
                            *FPaths::GetBaseFilename(PrefabPath));
                        return;
                    }

                    const FRotator Rotation = WorldRotationQuat.Rotator().GetNormalized();

                    if (DebugLoggedTransforms < 8)
                    {
                        UE_LOG(LogTemp, Display,
                            TEXT("CryImporter: GRP final '%s' id='%s' parent='%s' localPos=(%.3f, %.3f, %.3f) worldPos=(%.3f, %.3f, %.3f) worldRot=(P%.3f Y%.3f R%.3f) worldScale=(%.3f, %.3f, %.3f)"),
                            *FPaths::GetBaseFilename(PrefabPath),
                            *ObjectId,
                            *ParentId,
                    ...
                    if (Actor)
                    {
                        ...
                        if (!ObjectId.IsEmpty())
                        {
                            WorldTransformByObjectId.Add(ObjectId, FTransform(Rotation, Location, Scale3D));
                        }
                    }
                }
            }
